      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-gray-900 rounded-lg p-6 border border-gray-700">
            ...
            <p className="text-2xl font-bold text-foreground">{DELEGATES.length}</p>
          </div>
          ...
          <div className="bg-gray-900 rounded-lg p-6 border border-gray-700">
            ...
            <p className="text-2xl font-bold text-foreground">768</p>
          </div>
        </div>
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-xl text-foreground mb-2">{delegate.name}</h3>
                  <p className="text-sm text-gray-400 mb-3">{delegate.statement}</p>
                  <div className="flex flex-wrap gap-2 mb-3">
                    {delegate.communities.map((community) => (
                      <span
                        key={community}
                        className="px-2 py-1 text-xs font-medium bg-accent/20 text-accent-hover rounded"
                      >
                        {community}
                      </span>
                    ))}
              ...
              <div className="grid grid-cols-2 gap-4 mb-4 pt-4 border-t border-gray-700">
                <div>
                  ...
                  <p className="text-lg font-semibold text-foreground">{delegate.totalVotes}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Reputation</p>
                  <div className="flex items-center gap-2">
                    <Award className="w-4 h-4 text-yellow-500" />
                    <p className="text-lg font-semibold text-foreground">{delegate.reputation}</p>
                  </div>
                </div>
              </div>

              <button className="w-full px-4 py-2 bg-accent text-white rounded-lg font-semibold hover:bg-accent-hover transition-colors">
