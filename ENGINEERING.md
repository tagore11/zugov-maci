...
| **Zupoll-style**                | Off-chain, Semaphore-group-proof              | Deferred | Fills the "survey" decision-taking mechanism — not built yet, see TODOS.md                                                                                                        |
...
| 2026-08-23 | `useSiwe()` is a React Context (`SiweProvider`, mounted once in `app/providers.tsx`), not a per-call-site hook instance; `SiweGate`'s `siwe` prop removed                                                                                                                             | `/plan-eng-review` — up to 6 independent `useSiwe()` call sites each ran their own auto-sign-in effect, so a fresh wallet connect could fire 2-3 simultaneous `/api/auth/nonce` fetches racing to overwrite each other before `signIn()` finished. One Context instance makes "one nonce in flight at a time" true by construction instead of by convention. The backend's httpOnly SIWE cookie remains the sole source of truth for "signed in" — Privy/wagmi wallet-connected state is not authoritative (injected wallets like MetaMask can't even be programmatically disconnected). |
