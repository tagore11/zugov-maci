...
include "../../utils/trees/QuinaryCheckRootRanked.circom";
include "../../utils/trees/QuinaryGeneratePathIndices.circom";
include "../../utils/ranked/StateLeafAndBallotTransformer.circom";
 ...
 * This template supports Ranked Voting.
 */
template SingleMessageProcessorRanked(stateTreeDepth, voteOptionTreeDepth) {
    // Constants defining the structure and size of state and ballots.
    var STATE_LEAF_LENGTH = 3;
    var BALLOT_LENGTH = 2;
    var PACKED_COMMAND_LENGTH = 4;
    var VOTE_OPTION_TREE_ARITY = 5; //TODO: does this make sense any more?
    ...
    // at index MAX_RANKED_VOTE_OPTIONS.

    var computedCurrentVoteWeightPathIndices[voteOptionTreeDepth] = QuinaryGeneratePathIndices(voteOptionTreeDepth)(MAX_RANKED_VOTE_OPTIONS);
    ...
    // 5.1. Update the ballot's vote option root with the new votes (weighted).

    signal weightedVoteOptions[MAX_RANKED_VOTE_OPTIONS];
    
    // Assign all weighted vote options
    for (var i = 0; i < MAX_RANKED_VOTE_OPTIONS; i++) {
        weightedVoteOptions[i] <== voteWeightMux * rankedVotes[i];
    }

    var computedQuinaryRoot = QuinaryCheckRootRanked()(weightedVoteOptions, voteWeightMux);

    // The new vote option root in the ballot
    var newBallotVoteOptionRootMux = Mux1()(
     [ballot[BALLOT_VOTE_OPTION_ROOT_INDEX], computedQuinaryRoot],
        computedIsValid
    );
    ...
    var computedNewBallot = PoseidonHasher(2)([computedNewBallotNonce, newBallotVoteOptionRoot]);    // tpDO index set to 0
