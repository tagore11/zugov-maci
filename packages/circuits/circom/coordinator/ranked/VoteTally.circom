...
// local imports
include "../../utils/ranked/ResultCommitmentVerifier.circom";
...
template VoteTallyRanked(
    ...
    voteOptionTreeDepth // TODO: remove?
) {
    ...
    signal input votes[batchSize][MAX_RANKED_VOTE_OPTIONS];
    signal input voteWeights[batchSize];
    // Current results for each vote option.
    signal input currentResults[MAX_RANKED_VOTE_OPTIONS];
    ...
    // TODO: 
    for (var i = 0; i < batchSize; i++) {
        computedVoteTree[i] = QuinaryCheckRootRanked()(votes[i], voteWeights[i]);
    ...
    var computedCalculateTotalResult[MAX_RANKED_VOTE_OPTIONS];
    ...
    computedTotalVoiceCreditSpent[batchSize] = currentSpentVoiceCreditSubtotal * computedIsZero;

    for (var i = 0; i < batchSize; i++) {
        computedTotalVoiceCreditSpent[i] = voteWeights[i];
    }

    var computedNewSpentVoiceCreditSubtotal = CalculateTotal(batchSize + 1)(computedTotalVoiceCreditSpent);

    // Verifies the updated results and spent credits, ensuring consistency and correctness of tally updates.
    ResultCommitmentVerifierRanked(voteOptionTreeDepth)(
