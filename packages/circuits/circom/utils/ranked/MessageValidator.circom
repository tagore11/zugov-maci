...
template MessageValidatorRanked() {
    ...
    signal output rankedVotes[MAX_RANKED_VOTE_OPTIONS];
    ...
    // Check (2) - each vote option index must be less than the number of valid vote options
    var (votes[MAX_RANKED_VOTE_OPTIONS], computedIsUniqueAndValid) = UnpackRankedVoteOptionAndCheckUniqueness()(voteOptionIndex, voteOptions);
    rankedVotes <== votes;
