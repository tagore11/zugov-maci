 ...
 * This template supports the ranked mode.
 */
template StateLeafAndBallotTransformerRanked() {
    ...
    signal output votesWithWeight[MAX_RANKED_VOTE_OPTIONS];

    // Check if the command / message is valid.
    var (
        computedIsValid,
        computedIsStateLeafIndexValid,
        votes[MAX_RANKED_VOTE_OPTIONS]
    ) = MessageValidatorRanked()(
        commandStateIndex,
        totalSignups,
